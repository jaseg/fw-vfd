...
FONT = (
    ...
    (0x00,0x00,0x00,0x00,0x00), # blank (backspace, not handled)
    (0x0c,0x1e,0x3c,0x1e,0x0c), # heart
    (0x48,0x38,0x1e,0x38,0x48), # star
    (0x22,0x08,0x5d,0x08,0x22), # sun
    (0x3e,0x6c,0x78,0x6c,0x3e), # cat face
    (0x42,0x06,0x11,0x38,0x10), # stars
...
for i,ch in enumerate(FONT):
    print("/* 0x{:02x} */ {{0x{:02x},0x{:02x},0x{:02x},0x{:02x},0x{:02x}}}, /* '{}' */".format(
        i+0x20,
        *mapped(ch),
        chr(i+0x20) if i+0x20 < 0x7f else '.'))
